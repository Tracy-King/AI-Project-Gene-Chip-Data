...
if __name__ == "__main__":
    ...
    for i in range(0,5893):
        ...
        line_y = fy.readline().split('\t')[1:2][0]#[28:29][0]
        '''if line_y == '  ':
        ...
        trY.append(line_y)  
        trX.append(line_x)
        #if(len(line_y) != 194 or len(line_x) != 22283):
         #   print("Error:",i,len(line_y),len(line_x))
    ...
    dbn = DBN(n_in=1895, n_out=2, hidden_layers_sizes=[200, 200, 200])
    sess = tf.Session()
    with tf.device("/gpu:0"):
        ...
        dbn.finetuning(sess, trX[round(len(trX)/robin):], trY[round(len(trX)/robin):], teX, teY)
            #roundRobin(trX, robin)
            #roundRobin(trY, robin)
